...
class TaskQueue:
    def __init__(self, dsn, queue_name, table_name: str = 'task_queue', reset=False, ttl_zero_callback=None):
    ...
    def get(self):
        ...
        This statement marks the next available task in the queue as "processing" and returns its ID and task details. The query uses a FOR UPDATE SKIP LOCKED clause to lock the selected task so that other workers can't select the same task simultaneously.

        After executing the query, the method fetches the result using cur.fetchone(). If no task is found, the method returns None, None. Otherwise, it returns the task and its ID.

        Note that this method is non-blocking, which means it returns immediately even if there is no task available in the queue.. In order to mark that task as done, you have
        to use:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                """ UPDATE task_queue
                    SET processing = true,
                        deadline = NOW() + CAST(lease_timeout || ' seconds' AS INTERVAL)
                    WHERE id = (
                        SELECT id
                        FROM task_queue
    ...
    def _check_expired_leases(self):
        ...
        for row in expired_tasks:
            task_id = row[0]
            logger.info(f"Got expired task with type(id) {type(task_id)}")
    ...
    def _reset(self):
        """Delete all tasks in the DB prefixed with our queue name.
